...
function objective_f(
    c::AbstractVector, p::Tuple{<:Real,AbstractVector,AbstractMatrix}
)
    G = copy(p[3])
    G[:, (p[3]'*c.<=0)] .= 0
...
function gradient_f!(
    gradient, c, p::Tuple{<:Real,AbstractVector,AbstractMatrix}
)
    G = copy(p[3])
    G[:, (p[3]'*c.<=0)] .= 0
...
function hessian_f!(
    H, c, p::Tuple{<:Real,AbstractVector,AbstractMatrix}
)
    G = copy(p[3])
    G[:, (p[3]'*c.<=0)] .= 0
    H .= p[1] * I + G * p[3]'
